...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x = darknet.layers[92].output
    x = compose(
        MaxPooling2D(pool_size=(2,2),strides=(2,2),padding='same'),
        DarknetConv2D_BN_Leaky(512, (1, 1)),
        MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='same'),
        DarknetConv2D_BN_Leaky(1024, (1, 1)))(x)
    x = Concatenate()([x,darknet.output])
    x = DarknetConv2D(1024,(1,1))(x)
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    # if calc_loss == True:
    #     return grid, box_delta_xy, box_log_wh, box_sigma, box_confidence, box_class_probs
    if calc_loss == True:
        return grid, feats, box_xy, box_wh
    return box_xy, box_wh, box_confidence, box_class_probs
